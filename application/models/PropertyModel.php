   <?php
        Class PropertyModel extends CI_Model
		{
				...
				{
					return $this->db->select('property.*,administrator.name,category.category,subcategory.subcategory')
							->join('administrator','administrator.aid = property.user_id','left')
							->join('category','category.id = property.sell_type','left')
							->join('subcategory','subcategory.id = property.property_type','left')
							->order_by("property.created", "desc")
							->limit(5)
				...
				{
					$result = $this->db->where('request_from',3)->get('property')->num_rows();
				...
				{ 
					...
					if(!empty($data['dimension_measure']))
					{
					...
					if(!empty($data['min_rate']) || !empty($data['max_rate']))
					{
						...
						if($data['request_from'] ==  1 || $data['request_from'] ==  5 || $data['request_from'] == 6)
						{
						    $property['min_rates']    =  $data['min_rate'];	
					        $property['max_rate']    =  $data['max_rate'];
						}
						else
						{
							$property['min_rates']    =  $data['max_rate'];	
					        $property['max_rate']    =  $data['max_rate'];
						}
					}
